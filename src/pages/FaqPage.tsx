...
const QA: Array<{ q: string; a: string }> = [
  ...
  },
  {
    ...
    a: 'Sí. Al crear un dashboard recibes un “deleteToken” que se guarda en el almacenamiento local de tu navegador. Mientras conserves ese token (es decir, mientras no borres los datos del navegador) podrás eliminar el dashboard inmediatamente desde la propia interfaz. Si pierdes el token porque cambiaste de navegador o vaciaste la caché, escríbenos a contacto@exceltosky.example indicando el enlace y verificaremos manualmente la eliminación.',
  },
  {
    q: '¿Funciona offline?',
    a: 'En gran medida sí. Una vez has cargado la aplicación al menos una vez, el procesado de Excel y la navegación por los dashboards locales funcionan sin conexión, porque todo ocurre en tu navegador. Lo que no funciona offline es lo que necesita Internet por definición: compartir un nuevo dashboard, abrir un enlace público de otra persona, y la publicidad.',
  },
  {
    q: '¿Por qué hay anuncios?',
    a: 'Excel to Sky es un servicio gratuito y sin registro. Para cubrir los costes de servidor y mantenimiento, mostramos publicidad servida por Google AdSense en algunas zonas no críticas de la interfaz. Hemos elegido AdSense porque es transparente sobre las políticas de privacidad y respeta el consentimiento de cookies. Si en el futuro lanzamos un plan premium sin anuncios, lo anunciaremos en esta misma página.',
  },
  {
    q: '¿Puedo desactivar el adblock?',
    a: 'Puedes navegar con tu bloqueador de anuncios activado sin problemas: la aplicación seguirá funcionando con normalidad y no bloqueamos al usuario por usar adblock. Si quieres apoyar el proyecto, considéranos en la lista blanca de tu bloqueador (whitelist) para que carguen los anuncios y nos ayudes a mantener el servicio. Es totalmente voluntario.',
  },
  {
    q: '¿Cómo contactar?',
    a: 'Para cualquier consulta, sugerencia, problema técnico o solicitud relacionada con tus datos, escríbenos a contacto@exceltosky.example. Respondemos en español o inglés en un plazo de 1 a 3 días laborables. Si tu mensaje es urgente (por ejemplo, solicitas el borrado de un dashboard que contiene datos sensibles publicados por error), indícalo en el asunto con la etiqueta [URGENTE] y daremos prioridad a la petición.',
  },
]
